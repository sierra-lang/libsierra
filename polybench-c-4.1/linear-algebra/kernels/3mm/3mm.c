...
static void kernel_cont(int ni, int nj, int nk, int nl, int nm,
...
#pragma scop
    ...
    /* G := E*F */
    kernel_cont(ni, nj, nk, nl, nm, E, F, G);
/*for (i = 0; i < _PB_NI; i++)*/
/*for (j = 0; j < _PB_NL; j++) {*/
/*G[i][j] = SCALAR_VAL(0.0);*/
/*for (k = 0; k < _PB_NJ; ++k)*/
...
int main(int argc, char **argv) {
    ...
    fprintf(POLYBENCH_DUMP_TARGET,
            "=================== E =======================\n");
    polybench_prevent_dce(print_array(ni, nj, POLYBENCH_ARRAY(E)));
    fprintf(POLYBENCH_DUMP_TARGET,
            "\n\n=================== F =======================\n");
    polybench_prevent_dce(print_array(nj, nl, POLYBENCH_ARRAY(F)));
    fprintf(POLYBENCH_DUMP_TARGET, "\n\n");
