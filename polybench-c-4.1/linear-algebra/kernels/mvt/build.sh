...
opt -S -polly-opt-isl -polly-codegen -polly-task -polly-parallel $FILE.tmp $DEBUG -pass-remarks-analysis=polly-scops -polly-scops > $FILE-both-tile.ll 2> d.debug
...
opt -S -O3 $FILE-both-tile.ll > $FILE-bothend-tile.ll
...
clang -O3 $FILE-bothend-tile.ll -o $FILE-both-tile.exe $FLAGS -DPOLYBENCH_TIME $DATASETSIZE $DUMP
